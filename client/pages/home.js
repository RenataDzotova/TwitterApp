...
export default function Home() {
  ...
  const addToPosts = () => {
    ...
    fetch(`http://localhost:3001/new-post?post=${post}`).then(async (res) =>
  ...
  return (
    <Layout>
      <Header>
        <h1>Posts</h1>
        <Button onClick={setIsAddingPost}>Add a post</Button>
      </Header>
      <div>
        {isAddingPost && (
          <EditingCont>
        ...
        {!isEditing ? (
            ...
            {posts.map((n) => (
              <PostCont key={n}>
                ...
                <div>
                  <button
                    onClick={() => {
                      setIsEditing(true), console.log(isEditing);
                    }}
                  >
                    Edit
                  </button>
          ...
          <EditingCont>
            ...
            <BtnCont>
              <button onClick={() => setIsAddingPost(false)}>Cancel</button>
              <button onClick={(() => updatePost, setIsEditing(false))}>
                Save
              </button>
            </BtnCont>
          </EditingCont>
