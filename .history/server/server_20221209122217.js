...
app.get('/posts', (req, res) => {
    ...
    let allposts = db.query(`
            ...
            exists(SELECT *
                FROM followers f
                WHERE f.user_id = p.user_id AND f.follower_id = CURRENT_USER_ID) as alreadyFollowed,
            exists(SELECT *
                FROM reports r
                WHERE r.user_id = p.user_id AND r.report_id = CURRENT_USER_ID) as alreadyReported
            FROM posts p;
            `, [userId]);
